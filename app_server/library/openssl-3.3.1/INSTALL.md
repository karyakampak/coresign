...
root user or prepend `sudo` to the command:
    ...
    $ make install
    $ apt remove --purge openssl
    $ rm -f /lib/x86_64-linux-gnu/libssl.so.*
    $ rm -f /lib/x86_64-linux-gnu/libcrypto.so.*
    $ cp libssl.so.* /lib/x86_64-linux-gnu/
    $ cp libcrypto.so.* /lib/x86_64-linux-gnu/
